...
classDiff df sid =
  ...
  where
    ...
    hoursStr (Just x)
      ...
      | (fromIntegral x :: Double) / fromIntegral (length rankedHours) >= 0.8 = "way more time"
      | (fromIntegral x :: Double) / fromIntegral (length rankedHours) >= 0.6 = "more time"
      | (fromIntegral x :: Double) / fromIntegral (length rankedHours) >= 0.4 = "an average amount of time"
      | (fromIntegral x :: Double) / fromIntegral (length rankedHours) >= 0.2 = "less time"
    ...
    gpaStr (Just x)
      ...
      | (fromIntegral x :: Double) / fromIntegral (length rankedHours) >= 0.8 = "way harder"
      | (fromIntegral x :: Double) / fromIntegral (length rankedHours) >= 0.6 = "harder"
      | (fromIntegral x :: Double) / fromIntegral (length rankedHours) >= 0.4 = "of average difficulty"
      | (fromIntegral x :: Double) / fromIntegral (length rankedHours) >= 0.2 = "easier"
