...
navbarPartial = nav_ [class_ "flex items-center justify-between flex-wrap bg-teal-500 py-5 shadow-lg z-100 px-6 relative"] $ do
  div_ [class_ "lg:mx-auto lg:container w-full justify-between flex-wrap flex items-center"] $ do
    ...
    div_ [class_ "w-full block flex-grow lg:flex lg:items-center lg:w-auto"] $ do
      div_ [class_ "text-md lg:mx-auto"] $ do
        ...
        a_ [href_ "#", class_"block mt-4 lg:inline-block lg:mt-0 text-teal-200 hover:text-white mr-4"] "Test"
      div_ $ do
        a_ [href_ "#", class_ "inline-block text-sm px-4 py-2 leading-none border rounded text-white border-white hover:border-transparent hover:text-teal-500 hover:bg-white mt-4 lg:mt-0"] "Github"
